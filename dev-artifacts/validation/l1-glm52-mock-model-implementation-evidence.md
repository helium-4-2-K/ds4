...
- `serve/action-tp-group`
  - Code: `ds4_glm52_mock_tp4_step_add_rank` in `ds4_glm52_mock.c`, composed
    with the existing `ds4_glm52_tp4_rank_group` command/ack state machine.
  - Proves mock ranks share logical model/session identity, preserve
    ...
    cursor/KV state, and gather a coordinator-visible token from rank-local
    vocab candidates.
...
- The TP4 mock proof uses in-process rank contributions plus the existing
  rank-group state machine. It does not prove cross-machine network transport
  performance or real tensor all-reduce/gather math.
